...
export const distManhattan: Operator = {
    definition: {
        exec: function*() { throw new Error(`Operator 'distManhattan' can only be used inside a 'glsl' quotation.`); },
...
render`,
        ...
        expectedDescription: `A shader object rendering a box with colorful diamond-shaped ripples.`
    }, {
        code: `2 2 2 vec3 box
(
...
march render`,
        assert: (s) => s.length === 1 && s[0]?.type === 'shader',
        expectedDescription: `A shader object rendering a box with diamond-shaped ripples moving in a circle.`
    }]
};