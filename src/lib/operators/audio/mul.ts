...
export const mul: Operator = {
    definition: {
        ...
        description: 'Multiplies two audio signals (or a signal and a number for gain) by creating a combined quotation.',
        effect: '[L_quotationA L_quotationB_or_F] -> [L_quotation]'
    },
    examples: [
        { 
            code: "440 sine 0.5 mul 0.5 play",
            expected: []
        },
        { 
            code: "440 sine 0.5 mul",
            assert: (s) => s.length === 1 && Array.isArray(s[0]) && s[0].length === 3 && s[0][2] === 'mul'
        },
    ]
};