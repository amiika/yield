...
export const trunc: Operator = {
    definition: {
        exec: function*(s) {
            ...
            if (typeof fraction !== 'number' || fraction < 0 || fraction > 1) {
                throw new Error('trunc expects a fraction between 0 and 1.');
            }
            if (!isDurationalSequence(list)) {
                throw new Error('trunc expects a durational sequence from `dur`.');
            }
    ...
    examples: [
        {
            code: '(60 q 62 h 64 q) dur 0.5 trunc', // total dur = 1, target = 0.5
            expected: [ [[60, 0.25], [62, 0.25]] ]
        },
        {
            code: '(60 w) dur 0.75 trunc',
            expected: [ [[60, 0.75]] ]
        }
    ]
};
